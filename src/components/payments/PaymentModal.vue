...
const emit = defineEmits(['close', 'paymentAdded', 'addPayment']);
...
const processPayment = async () => {
  ...
  try {
    const success = await emit('addPayment', paymentData);
    if (success) {
      emit('paymentAdded');
      closeModal();
    } else {
      ...
      <div class="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
        ...
        <div class="sm:flex sm:items-start">
          <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
            <h3 class="text-xl font-semibold leading-6 text-gray-900">Process Payment</h3>
            <div class="mt-2">
              <p class="text-sm text-gray-500">
                Processing payment for {{ student.name }}
              </p>
            </div>
            <div class="mt-4">
