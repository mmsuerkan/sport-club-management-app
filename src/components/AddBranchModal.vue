...
const emit = defineEmits(['close', 'branchAdded']);
...
const addBranch = async () => {
  if (!name.value) {
    error.value = 'Please enter a branch name';
    return;
  }
  ...
  try {
    const newBranchRef = push(dbRef(db, 'branches'));
    ...
    error.value = 'Failed to add branch';
    console.error(e);
  } finally {
    loading.value = false;
  }
      ...
      <div class="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
        ...
        <div class="sm:flex sm:items-start">
          <div class="mt-3 text-center sm:mt-0 sm:text-left w-full">
            <h3 class="text-xl font-semibold leading-6 text-gray-900">Add New Branch</h3>
            <div class="mt-4">
